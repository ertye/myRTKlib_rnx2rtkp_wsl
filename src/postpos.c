                   ...
                   const char **infile, const int *index, int n, const char *outfile)
{
    ...
    for (i = 0; i < n; i++)
    { /* first check infiles for .BIA or .BSX files */
        if ((dcb_ok = readdcb(infile[i], &navs, stas)))
            break;
    }
    if (!dcb_ok && *fopt->dcb)
    { /* then check if DCB file specified */
    ...
    { /* combined or combined with no phase reset */
