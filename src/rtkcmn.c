...
extern void satno2id(int sat, char id[8])
{
    ...
    {
    case SYS_GPS:
        snprintf(id, 8, "G%02d", prn - MINPRNGPS + 1);
        return;
    case SYS_GLO:
        snprintf(id, 8, "R%02d", prn - MINPRNGLO + 1);
        return;
    case SYS_GAL:
        snprintf(id, 8, "E%02d", prn - MINPRNGAL + 1);
        return;
    case SYS_QZS:
        snprintf(id, 8, "J%02d", prn - MINPRNQZS + 1);
        return;
    case SYS_CMP:
        snprintf(id, 8, "C%02d", prn - MINPRNCMP + 1);
        return;
    case SYS_IRN:
        snprintf(id, 8, "I%02d", prn - MINPRNIRN + 1);
        return;
    case SYS_LEO:
        snprintf(id, 8, "L%02d", prn - MINPRNLEO + 1);
        return;
    case SYS_SBS:
        snprintf(id, 8, "%03d", prn);
        return;
    }
    strcpy(id, "");
}
...
static int cmpeph(const void *p1, const void *p2)
{
    ...
    if (q1->ttr.time != q2->ttr.time)
        return (int)(q1->ttr.time - q2->ttr.time);
    if (q1->toe.time != q2->toe.time)
...
static void sunpos_eci(gtime_t tutc, const double *erpv, double *rsun)
{
    ...
    if (fabs(timediff(tutc, tutc_)) < 1e-6)
    { // Check cache.
        for (int i = 0; i < 3; i++)
            rsun[i] = rsun_[i];
        return;
...
extern int getstapos(const char *file, const char *name, double *r)
{
    ...
    while (fgets(buff, sizeof(buff), fp))
    {
        ...
        if (state == 0)
        {
            ...
            char sname[256];
            double pos[3];
            if (sscanf(buff, "%lf %lf %lf %255s", pos, pos + 1, pos + 2, sname) < 4)
                continue;

            int i = 0;
            for (; sname[i] && name[i]; i++)
            {
                if (toupper(sname[i]) != toupper(name[i]))
                    break;
            }
