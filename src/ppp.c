                     ...
                     int f, const prcopt_t *opt, const obsd_t *obs)
{
    ...
    { /* add rcvr stdevs term */
        if (code)
            var += SQR(opt->err[7] * obs->Pstd[frq]);
        else
            var += SQR(opt->err[7] * obs->Lstd[frq] * 0.2);
    }
                      ...
                      double *Lc, double *Pc)
{
    ...
    for (i = 0; i < opt->nf; i++)
    {
        ...
        if (freq[i] == 0.0 || obs->L[i] == 0.0 || obs->P[i] == 0.0)
            continue;
        if (testsnr(0, 0, azel[1], obs->SNR[i], &opt->snrmask))
            continue;
    ...
    *Lc = *Pc = 0.0;
    ...
    if (L[0] != 0.0 && L[frq2] != 0.0)
        *Lc = C1 * L[0] + C2 * L[frq2];
    if (P[0] != 0.0 && P[frq2] != 0.0)
        *Pc = C1 * P[0] + C2 * P[frq2];
}
...
static void detslp_ll(rtk_t *rtk, const obsd_t *obs, int n)
{
    ...
    if (nf > NFREQ)
        nf = NFREQ; // Quieten compiler warnings on slip[] write.
    for (i = 0; i < n && i < MAXOBS; i++)
        for (j = 0; j < nf; j++)
        {
            if (obs[i].L[j] == 0.0 || !(obs[i].LLI[j] & (LLI_SLIP | LLI_HALFC)))
                continue;
...
static void update_stat(rtk_t *rtk, const obsd_t *obs, int n, int stat)
{
    ...
    for (i = 0; i < MAXSAT; i++)
        for (j = 0; j < opt->nf; j++)
        {
            if (rtk->ssat[i].slip[j] & (LLI_SLIP | LLI_HALFC))
                rtk->ssat[i].slipc[j]++;
